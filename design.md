...
- **13 Admin – Entitlements (FE-9.2):**
  - **Used, Pending and Remaining are left out**: `AdminEntitlement` only has the two stored numbers, and no endpoint gives an admin other people's used or pending days. In their place, a bold **Total** column (entitled + carried over), which updates while typing ("–" while a cell is invalid). The columns come back when the contract has the numbers.
  - **"Start new year" is left out**: it needs the remaining days (above), and the contract has no endpoint for it.
  - The type control lists the absence types that deduct from the balance, Vacation first. With a single type (the backend's seed: only Vacation deducts) it's hidden and the subtitle names the type: "Yearly vacation days per person for 2026, in steps of 0.5". The year pill offers last year to two years ahead, starting on the current one.
  - Everyone gets a row, also without an entitlement: both cells empty, placeholder "0". An empty cell counts as 0; **emptying both cells of a saved entitlement deletes it** (the footer says so: "Emptying both cells removes the entitlement: that person then has no vacation days in 2026.").
  - Cells are 96px number inputs (step 0.5, right-aligned). A value outside 0–366 or not in steps of 0.5 gets the 2px danger border and a message under the cell ("Use steps of 0.5, e.g. 2.5."), the Total shows "–" and **Save changes** is disabled.
  - The card's footer: the note on the left, then "● Unsaved changes" (primary) or a muted "Saved", then the primary **Save changes** ("Saving…"). Each changed row is one `PUT` (the contract's upsert). A 400 goes under its cell; other failures keep the row's edit and show a danger banner above the footer naming them ("Couldn’t save Carla Mendes. Something went wrong. Please try again.").
...
- **06 Timesheets – Export dialog (FE-8.1):**
  ...
  - **Preselection and coverage** (decision 33, "Update (after the demo)"): the preselected card is the client with the most hours that month, or Generic when they're all internal, or the user's client when the month has none. A choice the user makes sticks. When a client sheet leaves hours out, a line comes first in the same primary-soft warning, above the weeks line: "None of your 40 hours in September are on DKB projects, so this sheet will be empty. They are on DBIS projects." / "This sheet only has the 32 hours on DKB projects. Your other 7.5 hours in September are on Deka and internal projects." Not in the Figma frame yet.
...
- `BaseDialog`: overlay, 20px radius, title plus close icon, actions right-aligned (secondary, then primary). Widths 580px (forms), 520px (details) and 460px (confirmations, which have no title row: an icon, a heading and the actions). `ConfirmDialog` builds the 460px confirmation (round primary-soft icon, heading, text, secondary cancel and a primary or danger-fill confirm, an optional danger banner).
