...
For every feature:
1. **Together**: agree the questions, the screens and the **API contract** (endpoints, JSON, error codes). Jira label: `together`. The contract PR merges once CI is green, without waiting for the other dev (D34); adjustments go in a follow-up PR.
...
- Contract-first: openapi.yaml in the repo is the source of truth, merged in its `T` story PR before implementing (D34: it doesn't wait for the other dev)
- **Backend:**
